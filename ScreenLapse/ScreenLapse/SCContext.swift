...
class SCContext {
    ...
    // Time-lapse support
    static var timeLapseMultiplier: Int = 0   // 0 = normal; 5/10/15/30/60 = speed multiplier
    static var timeLapseFrameIndex: Int64 = 0 // synthetic frame counter; always increments
    ...
    static func performMicCheck() async {
        ...
        DispatchQueue.main.async {
            let alert = createAlert(title: "Permission Required",
                                                       message: "ScreenLapse needs permission to record your microphone.",
    ...
    private static func requestPermissions() {
        DispatchQueue.main.async {
            let alert = createAlert(title: "Permission Required",
                                                       message: "ScreenLapse needs screen recording permissions, even if you only intend on recording audio.",
                                                       button1: "Open Settings",
                                                       button2: "Cancel")
            if alert.runModal() == .alertFirstButtonReturn {
    ...
    static func requestCameraPermission() {
        ...
        case .denied:
            DispatchQueue.main.async {
                let alert = createAlert(title: "Permission Required",
                                                           message: "ScreenLapse needs this permission to record your camera or mobile device.",
                                                           button1: "Open Settings",
                                                           button2: "Cancel")
                if alert.runModal() == .alertFirstButtonReturn {
    ...
    static func stopRecording() {
        ...
        stream = nil
        let wasTimeLapse = timeLapseMultiplier > 0
        if ud.bool(forKey: "recordMic") && !wasTimeLapse {
        ...
        if streamType != .systemaudio {
            ...
            if !wasTimeLapse { if #available(macOS 13, *) { awInput.markAsFinished() }}
            vW.finishWriting {
                if vW.status != .completed {
                    ...
                    showNotification(title: "Failed to save file".local, body: "\(err)", id: "screenlapse.error.\(UUID().uuidString)")
                } else {
                        ...
                        mixAudioTracks(videoURL: filePath.url) { result in
                            ...
                            case .success(let url):
                                ...
                                if !ud.bool(forKey: "showPreview") {
                                    showNotification(title: "Recording Completed".local, body: String(format: "File saved to: %@".local, url.path), id: "screenlapse.completed.\(UUID().uuidString)")
                                }
                                DispatchQueue.main.async {
                ...
                Task {
                    ...
                    do {
                        ...
                        if !ud.bool(forKey: "showPreview") {
                            ...
                            let id = "screenlapse.completed.\(UUID().uuidString)"
                        ...
                        showNotification(title: "Failed to save file".local, body: "\(error.localizedDescription)", id: "screenlapse.error.\(UUID().uuidString)")
                    }
                }
            } else {
    ...
    // Rewrite a video frame's PTS/DTS to an arbitrary value. Used by time-lapse mode
    // to replace real-world timestamps with synthetic 30fps ones.
    static func remapTime(sample: CMSampleBuffer, to newPTS: CMTime, timescale: Int32 = 30) -> CMSampleBuffer? {
    ...
    static func mixAudioTracks(videoURL: URL, completion: @escaping (Result<URL, Error>) -> Void) {
        showNotification(title: "Still Processing".local, body: "Mixing audio track...".local, id: "screenlapse.processing.\(UUID().uuidString)")
