...
extension AppDelegate {
    func initVideo(conf: SCStreamConfiguration) {
        ...
        // Audio tracks are meaningless in time-lapse mode — skip them entirely.
        let isTimeLapse = SCContext.timeLapseMultiplier > 0
        if !isTimeLapse {
            if #available(macOS 13, *) {
    ...
    func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of outputType: SCStreamOutputType) {
        ...
        case .screen:
            ...
            // Time-lapse: replace the real-world PTS with a synthetic one at 30fps playback
            let isTimeLapse = SCContext.timeLapseMultiplier > 0
            if isTimeLapse {
                let syntheticPTS = CMTime(value: SCContext.timeLapseFrameIndex, timescale: 30)
                guard let remapped = SCContext.remapTime(sample: SampleBuffer, to: syntheticPTS) else { return }
                SampleBuffer = remapped
                SCContext.timeLapseFrameIndex += 1
            }

            if SCContext.vW != nil && SCContext.vW?.status == .writing, SCContext.startTime == nil {
                SCContext.startTime = Date.now
                let sessionStart = isTimeLapse ? CMTime.zero : CMSampleBufferGetPresentationTimeStamp(SampleBuffer)
                SCContext.vW.startSession(atSourceTime: sessionStart)
            }
            if !isTimeLapse && (SCContext.timeOffset.value > 0) {
            ...
            break
        case .audio:
            // Audio is meaningless at time-lapse speeds — skip entirely.
            if SCContext.timeLapseMultiplier > 0 { break }
