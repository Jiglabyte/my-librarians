...
class AppDelegate: NSObject, NSApplicationDelegate, SCStreamDelegate, SCStreamOutput, AVCaptureVideoDataOutputSampleBufferDelegate  {
    ...
    @AppStorage("timeLapseMultiplier")  var timeLapseMultiplier: Int = 0
    ...
    func applicationWillFinishLaunching(_ notification: Notification) {
        ...
        let process = NSWorkspace.shared.runningApplications.filter({ $0.bundleIdentifier == "com.screenlapse.app" })
        if process.count > 1 {
            DispatchQueue.main.async {
                let button = createAlert(title: "ScreenLapse is Running".local, message: "Please do not run multiple instances!".local, button1: "Quit".local).runModal()
    ...
    func applicationShouldHandleReopen(_ sender: NSApplication, hasVisibleWindows flag: Bool) -> Bool {
        if SCContext.stream == nil {
            ...
            if (!w1.isEmpty && w2.isEmpty) || w1.isEmpty {
                ...
                mainPanel.title = "ScreenLapse".local
...
func closeMainWindow() {
    for w in NSApp.windows.filter({ $0.title == "ScreenLapse".local }) {
        w.close()
    }
}
...
extension NSImage {
    static func createScreenShot() -> NSImage? {
        let excludedAppBundleIDs = ["com.screenlapse.app"]
