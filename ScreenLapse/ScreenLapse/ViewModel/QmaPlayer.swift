                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                        Task {
                            do {
                                ...
                                SCContext.showNotification(title: "Failed to save file".local, body: "\(error.localizedDescription)", id: "screenlapse.error.\(UUID().uuidString)")
                                return
                            }
                        }
                    }
...
extension UTType {
    static let qma = UTType(exportedAs: "com.screenlapse.app.qma")
}
