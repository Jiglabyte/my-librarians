    ...
    var body: some View {
        SForm {
            SGroupBox(label: "Startup") {
                ...
                SToggle("Show ScreenLapse on Dock", isOn: $showOnDock)
                    //.disabled(!showMenubar)
                SDivider()
                SToggle("Show ScreenLapse on Menu Bar", isOn: $showMenubar)
                    //.disabled(!showOnDock)
            }
...
struct RecorderView: View {
    ...
    var body: some View {
        SForm(spacing: 10) {
            ...
            SGroupBox {
                SToggle("Exclude ScreenLapse itself", isOn: $hideSelf)
                SDivider()
                if #available (macOS 13, *) {
...
struct OutputView: View {
        ...
        SForm(spacing: 30) {
            ...
            SGroupBox(label: "Time-Lapse") {
                ...
                if timeLapseMultiplier > 0 {
                    SDivider()
                    SItem(label: "Note") {
                        Text("Audio is automatically disabled in time-lapse mode.")
                            .font(.footnote)
                            .foregroundColor(.secondary)
                    }
                    SDivider()
                    SItem(label: "Estimate") {
                        VStack(alignment: .leading, spacing: 2) {
                            Text("10 min real  →  \(formatTimeLapseSec(600 / timeLapseMultiplier))")
                            Text("1 hr real      →  \(formatTimeLapseSec(3600 / timeLapseMultiplier))")
                            Text("8 hr real      →  \(formatTimeLapseSec(28800 / timeLapseMultiplier))")
                        }
    ...
    private func formatTimeLapseSec(_ sec: Int) -> String {
